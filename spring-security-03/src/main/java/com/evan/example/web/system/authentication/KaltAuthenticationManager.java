...
import com.evan.example.web.system.user.mapper.AuthenticationMapper;
...
@Slf4j
public class KaltAuthenticationManager implements AuthenticationManager {
    ...
    public KaltAuthentication authenticate(Authentication authentication) throws AuthenticationException {
        if (!(authentication instanceof KaltAuthentication)) {
            throw new AuthenticationServiceException("unsupported authentication type" + authentication.getClass().getSimpleName());
        }
        ...
        // 2. 查询用户
        User user = authenticationMapper.selectByUsername(username);
        ...
        List<GrantedAuthority> authorities = authenticationMapper.selectPermissionByUsername(username);
